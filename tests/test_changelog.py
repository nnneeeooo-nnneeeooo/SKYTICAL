...
def main() -> None:
    ...
    assert raw["updatedThrough"] == "2026-09-10"
    assert len(raw["entries"]) == 121

    historical = [row["commit"] for row in raw["entries"]
                  if row["commit"] is not None]
    assert len(historical) == 121
    ...
    for lang, view in (("zh", zh), ("en", en)):
        ...
        assert len(re.findall(r'class="changelog-entry"', html)) == 121
