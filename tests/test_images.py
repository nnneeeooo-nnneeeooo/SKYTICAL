"""Discoverable, process-isolated image regression tests.

Run with either python tests/test_images.py or
python -m pytest -q tests/test_images.py.

The original 63 checks are preserved verbatim in _images_legacy.py and
...
LEGACY = Path(__file__).with_name("_images_legacy.py")
EXPECTED_CHECKS = 63
...
if mode not in {"run", "fail", "network"}:
    raise ValueError("unknown images-test worker mode")
...
with ExitStack() as stack:
    ...
    if mode == "network":
        ...
        calls[0].args[0] = ast.Constant(
            "intentional image isolation regression failure"
        )
...
def _run_legacy(mode: str = "run"):
    ...
    with tempfile.TemporaryDirectory(prefix="skytical-images-test-") as directory:
        root = Path(directory)
        env.update(
            TMPDIR=str(root),
            TMP=str(root),
...
class ImagesIsolationTests(unittest.TestCase):
    ...
    def test_failure_propagates_to_test_runner(self):
        ...
        self.assertIn(
            "FAIL intentional image isolation regression failure", output
        )
        self.assertIn("62/63 passed, 1 FAILED", output)
        self.assertFalse(directory.exists())
