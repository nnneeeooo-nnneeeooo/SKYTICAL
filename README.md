...
- 航空／運輸相關性由程式在抓取後與發布前雙重檢查。軍事新聞必須直接涉及
  ...
  與官員視導不屬本站範圍。
- 三種編輯狀態：`publish`、`publish_brief`（短但可信的新事件）、`reject`
  （本次事件證據不足不發布，附具體理由）。航空安全與其他高風險題材仍保留
  `riskFlags`，但只要通過來源、逐字引文、事實綁定、用語與防虛構等程式閘門
  就直接發布；風險旗標本身不再建立人工審核佇列。
- 舊 `data/review.json` 僅供一次性遷移：新版管線會重新核驗仍在 14 天內的
  舊稿，合格者發布、不合格者不發布，所有結果保存至
  `data/review-archive.json` 後清空舊佇列。
...
prompt 撰寫；來源一致、逐字引文與稿件契約均通過後自動發布，來源衝突或
證據不足則拒絕發布。
